...
if ($_SERVER["REQUEST_METHOD"] != "GET") {
  ...
  $chosen_category_info_query = "SELECT * FROM gallery_images_and_categories WHERE id = $id";
  $chosen_category_info_result = mysqli_query($conn, $chosen_category_info_query);
  $chosen_category = mysqli_fetch_assoc($chosen_category_info_result);
} else {
  header('location: ../edit_gallery#gallery-edits.php');
  die();
}
?>
  ...
  <body>
    ...
    <section style='padding: 20px; width:100vw; display:flex; justify-content: center; align-content:center'>
        <div style='width: 100%; max-width: 525px;'>
            ...
            <h2 style='font-size:22px; padding: 30px;'>Edit <?= $chosen_category['gallery_category'] ?> category</h2>
            <form style="text-align: center; display:flex; flex-direction:column; gap 1rem; width:100%"
                ...
                <input style="margin-bottom:1rem" type="hidden" name="id" value="<?= $chosen_category['id'] ?>" />

                <div style="display: flex; flex-direction:column; font-size:22px; gap: 1rem">


                    <label for="gallery_img_alt">image description</label>
                    <input style="padding:10px; font-size: 22px" type="text" name="gallery_img_alt" id="gallery_img_alt"
                        value="<?= $chosen_category['gallery_img_alt'] ?>" />

                    <label for="gallery_category">image</label>
                    <input style=" font-size: 22px; padding:10px" type="file" name="image"
                        id="gallery_img" value="<?= $chosen_category['gallery_img'] ?>" />

                    <button style=" font-size: 22px" type="submit" name="submit_update_chosen_category_info">Update
                        <?= $chosen_category['gallery_category'] ?> Info</button>
                </div>
            </form>
        </div>
    </section>
